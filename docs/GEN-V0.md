...
기록 2026-09-20. 지위: D-035 3의 "기계 v2" 조건 구현과 첫 적용 결과. 입력은 위임문 q, 환경 초기 상태, `schema.py` 셋뿐이고 LLM을 쓰지 않는다. 숨은 규칙 R·규칙 유형·섭동표를 입력받지 않으며 "이 위임이 그 필드를 읽을 것인가"를 판단하지 않는다(`docs/DATA-GEN.md` §6 규칙 5). 코드는 `src/gen/minimal_pairs.py`, 실행기는 `scripts/gen_v0.py`. 이 문서의 표는 저자가 읽고 tier 1을 승인하거나 잘라 내기 위한 것이다. **아직 어떤 모델도 이 상태들을 돌리지 않았다.**
